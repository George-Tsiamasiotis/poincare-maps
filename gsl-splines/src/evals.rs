...
impl<'a> Spline<'a> {
    ...
    pub fn eval(&mut self, x: f64) -> Result<f64> {
        match self.gsl_spline.eval_e(x, &mut self.acc.gsl_iterp_accel) {
            Ok(val) => Ok(val),
    ...
    pub fn eval_deriv(&mut self, x: f64) -> Result<f64> {
        match self
            .gsl_spline
            .eval_deriv_e(x, &mut self.acc.gsl_iterp_accel)
        {
            Ok(val) => Ok(val),
            Err(err) => Err(SplineError::GSLInputDomainError { err }),
        }
    ...
    pub fn eval_deriv2(&mut self, x: f64) -> Result<f64> {
        match self
            .gsl_spline
            .eval_deriv2_e(x, &mut self.acc.gsl_iterp_accel)
        {
            Ok(val) => Ok(val),
            Err(err) => Err(SplineError::GSLInputDomainError { err }),
        }
    ...
    pub fn eval_integ(&mut self, a: f64, b: f64) -> Result<f64> {
        match self
            .gsl_spline
            .eval_integ_e(a, b, &mut self.acc.gsl_iterp_accel)
        {
            Ok(val) => Ok(val),
            Err(err) => Err(SplineError::GSLInputDomainError { err }),
        }
