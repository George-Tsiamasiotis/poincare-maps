...
// TODO: Init solver with uninit instead of initial
// TODO: Extract common k methods (if they are indeed the same)
...
mod solver {
    ...
    /// RK45 Classic method coefficients (Wikipedia)
    mod tableau {
    ...
    impl Solver {
        ...
        pub(crate) fn calculate_weights(&mut self) {
            for i in 0..self.weights.len() {
                self.weights[i] =
                    B1 * self.k1[i] + B2 * self.k2[i] + B3 * self.k3[i] + B4 * self.k4[i];
            }
        }
...
mod solver {
    ...
    impl Solver {
        pub(crate) fn init(&mut self, state: &State) {
            ...
            self.state3 = state.to_owned();
            self.state4 = state.to_owned();
            self.state5 = state.to_owned();
            self.state6 = state.to_owned();
            self.next = State::default();
        }
        ...
        pub(crate) fn start(
            ...
            self.calculate_state_k3(h, qfactor, bfield, current)?;
            self.calculate_state_k4(h, qfactor, bfield, current)?;
            self.calculate_state_k5(h, qfactor, bfield, current)?;
            self.calculate_state_k6(h, qfactor, bfield, current)?;
        ...
        pub(crate) fn calculate_state_k5(
            ...
            self.state5.evaluate(qfactor, current, bfield)?;
            self.k5 = [
        ...
        pub(crate) fn calculate_state_k6(
            ...
            self.state6.evaluate(qfactor, current, bfield)?;
            self.k6 = [
